...
<form action="{{ route('export.pdf') }}" method="post">
  ...
  <table>
    <thead>
      <tr>
        <th>Description</th><th>Amount</th><th>Category</th><th>Date</th><th>Currency</th>
      </tr>
    </thead>
...
<script>
  ...
  function stackedDatasets() {
    ...
    return [
      { label: 'Positive', data: pos, stack: 's', backgroundColor: 'rgba(34,197,94,0.6)', borderColor: 'rgba(34,197,94,1)' },
      { label: 'Negative', data: neg, stack: 's', backgroundColor: 'rgba(239,68,68,0.6)', borderColor: 'rgba(239,68,68,1)' },
    ];
  }
  ...
  function commonOptions(yTitle='Value') {
    return {
      ...
      plugins: {
        legend: { position: 'top' },
        tooltip: { callbacks: { label: ctx => `${ctx.dataset.label || ctx.label}: ${fmt.format(ctx.parsed)}` } }
      },
      scales: {
        x: { ticks: { autoSkip: true, maxRotation: 0 } },
        y: { beginAtZero: true, ticks: { callback: v => fmt.format(v) }, title: { display: true, text: yTitle } }
      }
    };
  }
  ...
  function buildConfig(kind) {
    const simpleOptions = {
      ...
      plugins: {
        legend: { position: 'top' },
        tooltip: { callbacks: { label: ctx => `${ctx.label}: ${fmt.format(ctx.parsed || ctx.raw)}` } }
      }
    };

    if (kind === 'horizontal') {
      return { type: 'bar', data: { labels, datasets: [barsDataset()] }, options: { ...commonOptions(), indexAxis: 'y' } };
    }
    if (kind === 'stacked') {
      return {
        type: 'bar',
    ...
    if (kind === 'line' || kind === 'area') {
      return {
        ...
        data: {
          labels,
          datasets: [{
            label: 'Totals', data: values,
            borderColor: 'rgba(59,130,246,1)', backgroundColor: 'rgba(59,130,246,0.2)',
            borderWidth: 2, pointRadius: 3, tension: 0.25, fill: (kind === 'area')
          }]
        },
    ...
    if (['pie','doughnut','radar','polarArea'].includes(kind)) {
      const typeMap = { pie:'pie', doughnut:'doughnut', radar:'radar', polarArea:'polarArea' };
      const ds = (kind === 'radar')
        ? [{ label: 'Totals', data: values, backgroundColor: 'rgba(59,130,246,0.2)', borderColor: 'rgba(59,130,246,1)' }]
        : [{ label: 'Totals', data: values, backgroundColor: labels.map((_, i) => baseColors[i % baseColors.length]) }];
      return { type: typeMap[kind], data: { labels, datasets: ds }, options: simpleOptions };
    }
    return { type: 'bar', data: { labels, datasets: [barsDataset()] }, options: commonOptions() };
  }
...
<style>
  /* add once */
  .chart-wrap {
    max-width: 700px;   /* keeps it compact on page/PDF */
    margin: 0 auto;
  }
  .chart-canvas {
    width: 100%;
    height: 380px;      /* final height you want in page/PDF */
  }
</style>
